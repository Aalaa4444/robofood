class Search:
    ...
    def get_neighbours(self, position, visited) -> list:
        ...
        if (position["x"] < len(self.robot.grille["table"]) - 1):
            if((position["x"] + 1, position["y"])not in visited):
                moves.append((position["x"] + 1, position["y"]))
        ...
        if position["y"] < len(self.robot.grille["table"][0]) - 1:
            if((position["x"], position["y"] + 1)not in visited):
                moves.append((position["x"], position["y"] + 1))
...
class AStar(Search):
    ...
    def get_h_score(grille, pos) -> float:
        ...
        for move in [(1, 0), (0, 1), (-1, 0), (0, -1)]:
            try:
                ...
                if grille["table"][pos[0] + move[0]][pos[1] + move[1]]:
    ...
    def search(self):
        ...
        while stack:
            ...
            if self.robot.grille["table"][current_node["x"]][current_node["y"]]:
